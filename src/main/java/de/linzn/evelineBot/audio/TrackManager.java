...
public class TrackManager extends AudioEventAdapter {
    ...
    public void queue(AudioTrack track, Member author) {
        EvelineBot.LOGGER.INFO("Add " + track.getInfo().title + " to queue. Requested by " + author.getEffectiveName());
    ...
    public void onTrackStart(AudioPlayer player, AudioTrack track) {
        EvelineBot.LOGGER.INFO("Start track " + track.getInfo().title);
    ...
    public void onPlayerPause(AudioPlayer player) {
        EvelineBot.LOGGER.INFO("Pause track " + player.getPlayingTrack().getInfo().title);
        this.mediaManager.updateInterface_TOP();
    ...
    public void onPlayerResume(AudioPlayer player) {
        EvelineBot.LOGGER.INFO("Resume track " + player.getPlayingTrack().getInfo().title);
        this.mediaManager.updateInterface_TOP();
    ...
    public void onTrackException(AudioPlayer player, AudioTrack track, FriendlyException exception) {
        EvelineBot.LOGGER.ERROR("ERROR while playing track " + track.getInfo().title + " with error " + exception.getLocalizedMessage());
        this.mediaManager.updateInterface_TOP();
    ...
    public void onTrackEnd(AudioPlayer player, AudioTrack track, AudioTrackEndReason endReason) {
        currentPlaying = null;
        if (!queue.isEmpty()) {
            EvelineBot.LOGGER.INFO("Track end with " + endReason + " and starting new one!");
            player.playTrack(this.queue.element().getTrack());
        } else {
            EvelineBot.LOGGER.INFO("Track end with " + endReason + "!");
            this.mediaManager.updateInterface_TOP();
            this.mediaManager.updateInterface_QUEUE();
        }
